...
- (void)viewDidLoad
{
	...
	sv.contentSize = CGSizeMake(10 * 480.0f*appdelegate.xval, sv.frame.size.height);
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    NSString *str = [appdelegate.birdlist objectAtIndex:pagecontrol.currentPage];
...
-(void) cbuttonclicked:(UIButton *)cbtn
{
  //  NSLog(@"Btn index==> %i", cbtn.tag);
    ...
    ////////////////////////////////
    NSString *newStr = [appdelegate.birdlist objectAtIndex:pagecontrol.currentPage];
...
- (void)zoomView:(UIButton *) tBtn
{
    
    NSString *str = [appdelegate.sound_bird objectAtIndex:pagecontrol.currentPage];
    [appdelegate playAudio:str :@"mp3"];
    ...
    if(currentPage>=1 && currentPage<10-1){
        ...
        for ( int tt = 0; tt < 10; tt++ ) {
            if ((scrollDirection==ScrollDirectionRight) && (tt < (currentPage-1) || tt > (currentPage+1)) && [sv viewWithTag:(100+(tt))] ) {
                ...
                NSString *birdname = (NSString *) [appdelegate.birdlist objectAtIndex:tt];
                //  NSLog(@"=====> %@",itemname);
                int len = [birdname length];
                for (int j=0; j<len; j++){
...
-(void) loadRestOfItem:(int)itemNo{
    ...
    NSString *str1 = [appdelegate.birdbg objectAtIndex:itemNo];
    ...
    NSString *str = [appdelegate.birdlist objectAtIndex:itemNo];
    tBtn.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:str]];
    tBtn.tag = 100+itemNo;
    [tBtn addTarget:self action:@selector(zoomView:) forControlEvents:(UIControlEventTouchUpInside)];
    [tBtn setOpaque:YES];
    [sv addSubview:tBtn];
    NSString *animallist = @"";
    animallist = (NSString *) [appdelegate.birdlist objectAtIndex:itemNo];
    // NSLog(@"=====> %@",animallist);
    ...
    for (int j=0; j<len; j++)
    {
        ...
        else if ([self.image isEqualToString:(@" ")])
        {
            k=15;
        }
        ...
        {
            UIView *cview=[[[UIView alloc]initWithFrame:CGRectMake(((itemNo*480)+totCharLen+first)*appdelegate.xval, 265*appdelegate.yval, k*appdelegate.xval, 50*appdelegate.yval)]autorelease];
            [sv addSubview:cview];
        }
