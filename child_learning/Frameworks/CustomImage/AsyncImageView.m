...
#define SPINNY_TAG 5555   
    ...
    //[spinny setFrame:CGRectMake(self.frame.size.width/2, self.frame.size.height/2, 20, 20)];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConnection {
    ...
    [self addSubview:imageView];
    imageView.frame = self.bounds;
   // imageView.frame = CGRectMake(5.0, 2.0, 56.0, 56.0);
	imageView.backgroundColor=[UIColor clearColor];
	//[imageView.image setImageOrientation:UIImageOrientationRight];
    [imageView setNeedsLayout]; // is this necessary if superview gets setNeedsLayout?
    [self setNeedsLayout];
    [data release];
    data = nil;
        //NSLog(@"async image entered");
